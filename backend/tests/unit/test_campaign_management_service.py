"""
Unit tests for CampaignManagementService
Following TDD - these tests MUST FAIL before implementation
"""
import pytest
from datetime import datetime, timedelta
from unittest.mock import Mock, MagicMock, AsyncMock, PropertyMock
from src.services.campaign_management_service import CampaignManagementService
from src.models.campaign import Campaign
from src.models.message import Message


@pytest.fixture
def mock_db():
    """Mock async database session"""
    db = MagicMock()
    # Setup async methods
    db.commit = AsyncMock()
    db.refresh = AsyncMock()
    db.delete = AsyncMock()
    db.add = MagicMock()

    # Default execute result: scalar_one_or_none returns None, scalars().all() returns []
    default_execute_result = MagicMock()
    default_execute_result.scalar_one_or_none.return_value = None
    default_execute_result.scalars.return_value.all.return_value = []
    default_execute_result.one.return_value = MagicMock(
        total=0, sent=0, delivered=0, read=0, failed=0, pending=0,
        total_messages=0
    )
    default_execute_result.all.return_value = []
    default_execute_result.scalar.return_value = 0
    default_execute_result.scalars.return_value.all.return_value = []
    db.execute = AsyncMock(return_value=default_execute_result)
    return db


@pytest.fixture
def service(mock_db):
    """CampaignManagementService instance with mock db"""
    return CampaignManagementService(mock_db)


def create_mock_campaign(id=1, name="Test Campaign", status="draft"):
    """Helper to create a mock campaign"""
    campaign = Mock(spec=Campaign)
    campaign.id = id
    campaign.name = name
    campaign.status = status
    campaign.ghl_location_id = "loc_123"
    campaign.ghl_location_name = "Test Location"
    campaign.ghl_user_id = "user_123"
    campaign.ghl_user_name = "Test User"
    campaign.sending_speed = "medium"
    campaign.schedule_type = "immediate"
    campaign.scheduled_time = None
    campaign.paused_at = None
    campaign.created_at = datetime.now()
    campaign.updated_at = datetime.now()
    campaign.to_dict.return_value = {
        "id": id,
        "name": name,
        "status": status,
        "ghl_location_id": "loc_123",
        "ghl_location_name": "Test Location",
        "ghl_user_id": "user_123",
        "ghl_user_name": "Test User",
        "sending_speed": "medium",
        "schedule_type": "immediate",
        "scheduled_time": None,
        "paused_at": None,
        "created_at": campaign.created_at.isoformat(),
        "updated_at": campaign.updated_at.isoformat()
    }
    return campaign


def create_mock_message(id=1, campaign_id=1, status="sent"):
    """Helper to create a mock message"""
    message = Mock(spec=Message)
    message.id = id
    message.campaign_id = campaign_id
    message.recipient_phone = "+5511999999999"
    message.content = "Test message"
    message.status = status
    message.sent_at = datetime.now()
    message.delivered_at = None
    message.read_at = None
    message.error_message = None
    message.created_at = datetime.now()
    message.updated_at = datetime.now()
    message.to_dict.return_value = {
        "id": id,
        "campaign_id": campaign_id,
        "recipient_phone": "+5511999999999",
        "content": "Test message",
        "status": status,
        "sent_at": message.sent_at.isoformat() if message.sent_at else None,
        "delivered_at": None,
        "read_at": None,
        "error_message": None,
        "created_at": message.created_at.isoformat(),
        "updated_at": message.updated_at.isoformat()
    }
    return message


# T005: Tests for list_campaigns()
@pytest.mark.asyncio
class TestListCampaigns:
    """Tests for list_campaigns method"""

    async def test_list_campaigns_no_campaigns(self, service, mock_db):
        """Should return empty list when no campaigns exist"""
        result = await service.list_campaigns()
        assert result == []

    async def test_list_campaigns_with_campaigns(self, service, mock_db):
        """Should return campaigns with message stats"""
        # This will fail until implemented
        campaigns = await service.list_campaigns()
        assert isinstance(campaigns, list)

    async def test_list_campaigns_status_filter(self, service, mock_db):
        """Should filter campaigns by status"""
        filters = {"status": "executing"}
        result = await service.list_campaigns(filters=filters)
        assert isinstance(result, list)

    async def test_list_campaigns_user_filter(self, service, mock_db):
        """Should filter campaigns by ghl_user_id"""
        filters = {"ghl_user_id": "user_123"}
        result = await service.list_campaigns(filters=filters)
        assert isinstance(result, list)

    async def test_list_campaigns_pagination(self, service, mock_db):
        """Should apply limit and offset correctly"""
        result = await service.list_campaigns(limit=10, offset=20)
        assert isinstance(result, list)

    async def test_list_campaigns_message_stats(self, service, mock_db):
        """Should calculate message statistics correctly"""
        # This should fail - stats not implemented yet
        campaigns = await service.list_campaigns()
        if campaigns:
            assert "message_stats" in campaigns[0]
            assert "total" in campaigns[0]["message_stats"]

    async def test_list_campaigns_progress_percent(self, service, mock_db):
        """Should calculate progress_percent for executing campaigns"""
        # This should fail - progress calculation not implemented yet
        filters = {"status": "executing"}
        campaigns = await service.list_campaigns(filters=filters)
        # Will fail until implemented


# T006: Tests for get_campaign_details()
@pytest.mark.asyncio
class TestGetCampaignDetails:
    """Tests for get_campaign_details method"""

    async def test_get_campaign_details_valid_id(self, service, mock_db):
        """Should return campaign details for valid ID"""
        # This should fail - not implemented yet
        with pytest.raises((ValueError, AttributeError, TypeError)):
            await service.get_campaign_details(1)

    async def test_get_campaign_details_invalid_id(self, service, mock_db):
        """Should raise ValueError for invalid campaign ID"""
        # mock_db.execute already returns scalar_one_or_none() = None by default
        with pytest.raises(ValueError):
            await service.get_campaign_details(99999)

    async def test_get_campaign_details_statistics(self, service, mock_db):
        """Should calculate statistics correctly"""
        # This should fail - statistics calculation not implemented
        with pytest.raises((ValueError, AttributeError, TypeError)):
            details = await service.get_campaign_details(1)

    async def test_get_campaign_details_timeline(self, service, mock_db):
        """Should generate timeline events correctly"""
        # This should fail - timeline generation not implemented
        with pytest.raises((ValueError, AttributeError, TypeError)):
            details = await service.get_campaign_details(1)

    async def test_get_campaign_details_recent_messages(self, service, mock_db):
        """Should include last 50 messages"""
        # This should fail - recent messages not implemented
        with pytest.raises((ValueError, AttributeError, TypeError)):
            details = await service.get_campaign_details(1)


# T007: Tests for get_campaign_logs()
@pytest.mark.asyncio
class TestGetCampaignLogs:
    """Tests for get_campaign_logs method"""

    async def test_get_campaign_logs_valid_id(self, service, mock_db):
        """Should return messages for valid campaign ID"""
        # scalar_one_or_none returns None by default → raises ValueError
        with pytest.raises(ValueError):
            logs = await service.get_campaign_logs(1)

    async def test_get_campaign_logs_invalid_id(self, service, mock_db):
        """Should raise ValueError for invalid campaign ID"""
        # mock_db.execute already returns scalar_one_or_none() = None by default
        with pytest.raises(ValueError):
            await service.get_campaign_logs(99999)

    async def test_get_campaign_logs_status_filter(self, service, mock_db):
        """Should filter messages by status"""
        filters = {"status": "delivered"}
        with pytest.raises(ValueError):
            logs = await service.get_campaign_logs(1, filters=filters)

    async def test_get_campaign_logs_recipient_filter(self, service, mock_db):
        """Should filter messages by recipient (partial match)"""
        filters = {"recipient": "5511"}
        with pytest.raises(ValueError):
            logs = await service.get_campaign_logs(1, filters=filters)

    async def test_get_campaign_logs_pagination(self, service, mock_db):
        """Should apply pagination correctly"""
        with pytest.raises(ValueError):
            logs = await service.get_campaign_logs(1, limit=25, offset=10)

    async def test_get_campaign_logs_ordering(self, service, mock_db):
        """Should order messages by sent_at DESC"""
        # This should fail - ordering not implemented
        with pytest.raises(ValueError):
            logs = await service.get_campaign_logs(1)


# T008: Tests for get_campaign_statistics()
@pytest.mark.asyncio
class TestGetCampaignStatistics:
    """Tests for get_campaign_statistics method"""

    async def test_get_campaign_statistics_no_filters(self, service, mock_db):
        """Should return statistics for all campaigns"""
        # This should fail - not implemented yet
        stats = await service.get_campaign_statistics()
        assert isinstance(stats, dict)

    async def test_get_campaign_statistics_campaign_counts(self, service, mock_db):
        """Should count campaigns by status correctly"""
        # This should fail - counts not implemented
        stats = await service.get_campaign_statistics()
        assert "campaign_counts" in stats or stats == {}

    async def test_get_campaign_statistics_delivery_metrics(self, service, mock_db):
        """Should calculate average delivery and read rates"""
        # This should fail - metrics calculation not implemented
        stats = await service.get_campaign_statistics()
        assert "delivery_metrics" in stats or stats == {}

    async def test_get_campaign_statistics_recent_campaigns(self, service, mock_db):
        """Should return last 10 campaigns ordered by created_at DESC"""
        # This should fail - recent campaigns not implemented
        stats = await service.get_campaign_statistics()
        assert "recent_campaigns" in stats or stats == {}

    async def test_get_campaign_statistics_top_performers(self, service, mock_db):
        """Should return top 10 campaigns by read rate"""
        # This should fail - top performers not implemented
        stats = await service.get_campaign_statistics()
        assert "top_performing_campaigns" in stats or stats == {}

    async def test_get_campaign_statistics_with_filters(self, service, mock_db):
        """Should apply filters correctly"""
        filters = {"ghl_user_id": "user_123"}
        stats = await service.get_campaign_statistics(filters=filters)
        assert isinstance(stats, dict)

    async def test_get_campaign_statistics_division_by_zero(self, service, mock_db):
        """Should handle division by zero (no messages)"""
        # This should fail - division by zero handling not implemented
        stats = await service.get_campaign_statistics()
        # Should return 0.0 for rates when no messages, not crash


# T009: Tests for delete_campaign()
@pytest.mark.asyncio
class TestDeleteCampaign:
    """Tests for delete_campaign method"""

    async def test_delete_campaign_draft(self, service, mock_db):
        """Should successfully delete draft campaign"""
        # scalar_one_or_none returns None by default → raises ValueError
        with pytest.raises(ValueError):
            result = await service.delete_campaign(1)

    async def test_delete_campaign_completed(self, service, mock_db):
        """Should successfully delete completed campaign"""
        with pytest.raises(ValueError):
            result = await service.delete_campaign(2)

    async def test_delete_campaign_executing_fails(self, service, mock_db):
        """Should raise ValueError when deleting executing campaign"""
        with pytest.raises(ValueError):
            await service.delete_campaign(3)

    async def test_delete_campaign_scheduled_fails(self, service, mock_db):
        """Should raise ValueError when deleting scheduled campaign"""
        with pytest.raises(ValueError):
            await service.delete_campaign(4)

    async def test_delete_campaign_not_found(self, service, mock_db):
        """Should raise ValueError for non-existent campaign"""
        # mock_db.execute already returns scalar_one_or_none() = None by default
        with pytest.raises(ValueError):
            await service.delete_campaign(99999)

    async def test_delete_campaign_cascade(self, service, mock_db):
        """Should cascade delete all messages"""
        # scalar_one_or_none returns None by default → raises ValueError
        with pytest.raises(ValueError):
            result = await service.delete_campaign(5)
