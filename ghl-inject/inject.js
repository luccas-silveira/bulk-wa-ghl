...
(function () {
  ...
  // ---------- Injeção via MutationObserver ----------
  function findEmpresasTab() {
  ...
  function tryInject() {
    ...
    const empresas = findEmpresasTab();
    if (!empresas) return;
  ...
  function tryRemove() {
    // Remove botão e overlay se "Empresas" sumiu do DOM (saiu de Contacts)
    if (!findEmpresasTab()) {
  ...
  // Tenta imediatamente caso o nav já esteja no DOM
  tryInject();
})();
